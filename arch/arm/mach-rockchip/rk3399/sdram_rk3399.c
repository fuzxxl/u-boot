		    ...
		    const struct rk3399_sdram_params *sdram_params)
{
	 ...
	 */
//	sitopwatch_init_msecs_expire(&sw, 100);
	tmp = 0;
...
static void dram_all_config(const struct rk3399_sdram_params *sdram_params)
{
	...
	debug("%s sysreg %x, %x\n", __func__, sys_reg, &rk3399_pmugrf->os_reg2);
...
#define CONFIG_DRAM_SIZE_MB 4*1024*1024*1024
...
size_t sdram_size_mb(void)
{
	...
	if (!size_mb) {
		 ...
		 * so we reserve it
		 */
		size_mb = MIN(size_mb, 0xf8000000/(1<<20));
	}
...
#ifdef CONFIG_SPL_BUILD
	.platdata_auto_alloc_size = sizeof(struct rk3399_sdram_params),
#endif
};
